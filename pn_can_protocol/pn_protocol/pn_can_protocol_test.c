	...
	while (1) {
		...
		if ((tock - tick) >= 100) {
			...
			StaticCanProtocol.addTxMessagePtr(&link1, 0xC1, tx_bytes3,
					sizeof(tx_bytes3));
			StaticCanProtocol.addTxMessagePtr(&link2, 0xD1, tx_bytes4,
					sizeof(tx_bytes4));
			StaticCanProtocol.addTxMessagePtr(&link2, 0xE1, tx_bytes3,
					sizeof(tx_bytes3));
			tick = tock;
		};
...
static void runTx() {
	...
	heap = StaticBuddyHeap.new(buffer, sizeof(buffer), 4);
	StaticCanProtocol.addLink(&heap, &link1, canSend, txCallback1, rxCallback1,
			0);
	StaticCanProtocol.addLink(&heap, &link2, canSend, txCallback2, rxCallback2,
			0);
	canInit();
...
//			StaticBuddyHeap.printMap(heap);
		...
		if ((tock - tick) >= 10) {
			...
			StaticCanProtocol.addTxMessagePtr(&link1, 0xC, tx_bytes3,
					sizeof(tx_bytes3));
			StaticCanProtocol.addTxMessagePtr(&link2, 0xD, tx_bytes4,
					sizeof(tx_bytes4));
